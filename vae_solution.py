...
class DiagonalGaussianDistribution(object):
  ...
  def kl(self):
    ...
    kl_div = 0.5 * (self.mean ** 2 + self.var - 1 - self.logvar)
    kl_div = kl_div.sum(dim=1)
    # refer to https://mr-easy.github.io/2020-04-16-kl-divergence-between-2-gaussian-distributions/ 
    return kl_div
  ...
  def nll(self, sample, dims=[1, 2, 3]):
    ...
    negative_ll = 0.5 * (torch.log(2 * np.pi * self.var) + (sample - self.mean) ** 2 / self.var)
...
class VAE(nn.Module):
  ...
  def log_likelihood(self, x, K=100):
    ...
    for i in range(K):
      ...
      log_likelihood[:, i] = - (prior.nll(z, dims=1) + recon.nll(x) - posterior.nll(z, dims=1))     # WRITE CODE HERE (log of the summation terms in approximate log-likelihood, that is, log p_\theta(x, z_i) - log q_\phi(z_i | x))
      # prior + reconstruct - posterior
      del z, recon
    
    ll = -torch.log(torch.tensor(K, dtype=torch.float).to(self.device)) + torch.logsumexp(log_likelihood, dim=1)      # WRITE CODE HERE (compute the final log-likelihood using the log-sum-exp trick)
    return ll
...
if __name__ == '__main__':
  ...
  save_image((test_image + 1.) * 0.5, f'./results/reconstruct/ground_truth.png')
  # load model
  for epoch in range(epochs):
    ...
    model.eval()
    with torch.no_grad():
      recon, _, _ = model(test_image)
      save_image((recon + 1.) * 0.5, './results/reconstruct/recon_{epoch}.png')
      
    if epoch % 5 == 4:
      samples = model.sample(batch_size=64)
      save_image((samples + 1.) * 0.5, f'./results/sample/samples_{epoch}.png')
...
def interpolate(model, z_1, z_2, n_samples):
  ...
  z = z_2 + (z_1 - z_2) * lengths    # WRITE CODE HERE (interpolate z_1 to z_2 with n_samples points)
  # z = z_1 + (z_2 - z_1) * lengths
